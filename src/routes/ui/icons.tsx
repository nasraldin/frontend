...
export default function IconsPage() {
  ...
  const filteredSvgIcons = createMemo(() => {
    ...
    if (!search) return svgIcons;
    return svgIcons.filter((name) => name.toLowerCase().includes(search));
  });

  const filteredFlagIcons = createMemo(() => {
    const search = flagsSearch().toLowerCase();
    if (!search) return flagIcons;
    return flagIcons.filter((name) => name.toLowerCase().includes(search));
  });
    ...
    <main class="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white">
      ...
      <section class="relative flex h-screen items-center justify-center overflow-hidden">
        <ParallaxScroll speed={0.05} direction="up" className="absolute inset-0">
          <div class="h-full w-full bg-gradient-to-r from-green-600/20 to-emerald-600/20" />
        </ParallaxScroll>
          ...
          <FadeInSection direction="up" className="mb-8">
            <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              ...
              <div class="flex items-center gap-4">
                ...
                <div class="flex items-center gap-2">
                  <label class="text-sm text-gray-300">Custom:</label>
                  <input
                    type="number"
                    min="8"
                    max="128"
                ...
                <div class="flex items-center gap-2">
                  <label class="text-sm text-gray-300">Direction:</label>
                  <button
        ...
        <div class="mx-auto max-w-6xl">
          ...
          <div class="grid gap-4 md:grid-cols-5 lg:grid-cols-6">
            {displayedSvgIcons().map((name, index) => (
              <FadeInSection
                direction="up"
                delay={index * 0.05}
                className="group cursor-pointer rounded-lg bg-gradient-to-br from-green-500/20 to-emerald-500/20 p-4 text-center transition-all duration-300 hover:scale-105 hover:from-green-500/30 hover:to-emerald-500/30"
              >
                <div onClick={() => setSelectedSvg(name)}>
                  <div class="mb-2 flex justify-center">
                    <img
                      src={`/svg/${name}.svg`}
                      alt={name}
          ...
          <div class="mt-8 text-center">
            <p class="text-gray-400">
              ...
              SVG icons
              {svgSearch() && ` (filtered from ${svgIcons.length} total)`}
            </p>
        ...
        <div class="mx-auto max-w-6xl">
          ...
          <div class="grid gap-4 md:grid-cols-5 lg:grid-cols-6">
            {displayedFlagIcons().map((name, index) => (
              <FadeInSection
                direction="up"
                delay={index * 0.05}
                className="group rounded-lg bg-gradient-to-br from-blue-500/20 to-cyan-500/20 p-4 text-center transition-all duration-300 hover:scale-105 hover:from-blue-500/30 hover:to-cyan-500/30"
              >
                <div class="mb-2 flex justify-center">
                  <img
                    src={`/flags/${name}.svg`}
          ...
          <div class="mt-8 text-center">
            <p class="text-gray-400">
              ...
              flag icons
              {flagsSearch() && ` (filtered from ${flagIcons.length} total)`}
            </p>
            <p class="mt-2 text-sm text-gray-500">
              Country and region flags from public/flags directory
            </p>
        ...
        <div class="mx-auto max-w-6xl">
          ...
          <div class="grid gap-8 md:grid-cols-3" dir={isRTL() ? 'rtl' : 'ltr'}>
            <FadeInSection
              direction="up"
              delay={0.1}
              className="rounded-lg bg-gradient-to-br from-blue-500/20 to-cyan-500/20 p-8"
            >
            ...
            <FadeInSection
              direction="up"
              delay={0.2}
              className="rounded-lg bg-gradient-to-br from-green-500/20 to-emerald-500/20 p-8"
            >
            ...
            <FadeInSection
              direction="up"
              delay={0.3}
              className="rounded-lg bg-gradient-to-br from-purple-500/20 to-pink-500/20 p-8"
            >
